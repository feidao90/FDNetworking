...
@implementation VOCacheManager
+ (VOCacheManager *)shareManager {
    static VOCacheManager *_VOCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _VOCacheManager = [[VOCacheManager alloc] init];
    });
    return _VOCacheManager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [VOMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"VONetworking"] stringByAppendingPathComponent:@"networkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [VODiskCache writeData:data toDir:directoryPath filename:hash];
        
        [[VOLRUManager shareManager] addFileNode:hash];
    }
...
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [VOMemoryCache readDataWithKey:hash];
    
    if (!cacheData) {
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        
        if (directoryPath) {
            cacheData = [VODiskCache readDataFromDir:directoryPath filename:hash];
            
            if (cacheData) [[VOLRUManager shareManager] refreshIndexOfFileNode:hash];
        }
    }
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"VONetworking"] stringByAppendingPathComponent:@"download"];
    ...
    [VODiskCache writeData:data toDir:directoryPath filename:fileName];
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) data = [VODiskCache readDataFromDir:directoryPath filename:fileName];
...
- (NSUInteger)totalCacheSize {
    ...
    return [VODiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    ...
    [VODiskCache clearDataIinDir:diretoryPath];
}
...
- (void)clearTotalCache {
    ...
    [VODiskCache clearDataIinDir:directoryPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray *deleteFiles = [[VOLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [VODiskCache deleteCache:filePath];
            }];
